...
class bdc_2_unify(nn.Module):
    def __init__(self, img_channels=3, num_resblocks=5, num_channels=64):
        super(bdc_2_unify, self).__init__()
        self.num_channels = num_channels
        self.pwcnet = PWCNet()

        self.forward_rnn = DBSNl(in_ch=img_channels*2 + num_channels, out_ch=num_channels, base_ch=num_channels, num_module=num_resblocks)
        self.backward_rnn = DBSNl(in_ch=img_channels*2 + num_channels, out_ch=num_channels, base_ch=num_channels, num_module=num_resblocks)
    ...
    def forward_testing(self, seqn, noise_level_map=None):
        ...
        forward_h = self.forward_rnn(torch.cat((seqn[:, 0], init_forward_h, noise_level_map[:, 0]), dim=1))
        ...
        for i in range(1, T):
            ...
            forward_h = self.forward_rnn(torch.cat((seqn[:, i], aligned_forward_h, noise_level_map[:, i]), dim=1))
        ...
        backward_h = self.backward_rnn(torch.cat((seqn[:, -1], init_backward_h, noise_level_map[:, -1]), dim=1))
        ...
        for i in range(2, T+1):
            ...
            backward_h = self.backward_rnn(torch.cat((seqn[:, T-i],  aligned_backward_h, noise_level_map[:, T-i]), dim=1))
