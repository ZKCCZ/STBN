...
def fastdvdnet_batch_psnr(img, imclean, data_range=1.):
    ...
    # return compare_psnr(imgclean.clip(0, 1), img_cpu, data_range=data_range)
    for i in range(img_cpu.shape[0]):
        psnr += compare_psnr(imgclean[i].clip(0,1), img_cpu[i], data_range=data_range)
    return psnr/img_cpu.shape[0]
    # for i in range(img_cpu.shape[1]):
    #     psnr += compare_psnr(imgclean[:,i, :, :, :].clip(0,1), img_cpu[:,i, :, :, :].clip(0,1), data_range=data_range)
    # return psnr/img_cpu.shape[1]
...
def normalize_augment(img_train,aug=False):
    ...
    def transform(sample):
        ...
        w_aug = [32, 12, 12, 12, 12, 12, 12, 12, 0] # one fourth chances to do_nothing
