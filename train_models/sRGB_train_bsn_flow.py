...
# from models import  BiRNN, FloRNN, ForwardRNN_b2u
...
def main(**args):
    ...
    optimizer.add_param_group({'params': model.module.pwcnet.parameters(), 'lr': 1e-5})
    ...
    for epoch in range(start_epoch, args['epochs']):
        ...
        for i, data in enumerate(loader_train):
            ...
            seqn = seq + noise
            # N2N
            seqdn, sigma,loss_flow = model(seqn, noise_level_map)
            sigma = stdn.squeeze() if sigma == None else sigma
            loss = loss_function_batch(seqdn, seqn, mode="loglike", sigma=sigma)+loss_flow
            ...
            #     if p.requires_grad:  # 确保参数没有被冻结
            #         if p.grad is not None:
            #             print(f"Before clipping, gradient norm of {name}: {p.grad.norm()}")
            #             print(f"After clipping, gradient norm of {name}: {p.grad.norm()}")
            #         else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--noise_ival", nargs=2, type=int, default=[30, 30])
