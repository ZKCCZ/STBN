...
from scipy.stats import norm, gaussian_kde, poisson
import cv2
import numpy as np
import os
import torch
...
class CRVDTrainDataset(Dataset):
    def __init__(self, CRVD_path, patch_size, patches_per_epoch, mirror_seq=False):
        ...
        for iso in iso_list:
            for scene_id in self.scene_id_list:
                self.seqs['%d_%d_clean' % (iso, scene_id)] = open_CRVD_seq_raw(os.path.join(self.CRVD_path, 'indoor_raw_gt/indoor_raw_gt_scene%d/scene%d/ISO%d' % (scene_id, scene_id, iso)),
                                                                               'frame%d_clean_and_slightly_denoised.tiff')
                for i in range(10):
                    self.seqs['%d_%d_noisy_%d' % (iso, scene_id, i)] = open_CRVD_seq_raw(os.path.join(self.CRVD_path, 'indoor_raw_noisy/indoor_raw_noisy_scene%d/scene%d/ISO%d' % (scene_id, scene_id, iso)),
    ...
    def __getitem__(self, index):
        ...
        # print(iso, scene_id, noisy_index)
        ...
        # noise = generate_noisy_raw(gt_raw, a, b)
        return {'seq': seq,
                'seqn': seqn,
                'a': a, 'b': b}
...
def generate_noisy_raw(raw, a, b):
    a /= (2 ** 12 - 1 - 240)
    gaussian_noise_var = b / ((2 ** 12 - 1 - 240) ** 2)

    poisson_noisy_img = Poisson(raw / a).sample() * a
    gaussian_noise = torch.sqrt(torch.tensor(gaussian_noise_var)) * torch.randn(raw.shape)

    noisy_img = poisson_noisy_img + gaussian_noise  # + 240
    return noisy_img - raw  # , std

# failure


class CRVDER2RDataset(Dataset):
    def __init__(self, CRVD_path, patch_size, patches_per_epoch, mirror_seq=False):
        self.CRVD_path = CRVD_path
        self.patches_per_epoch = patches_per_epoch
        self.patch_size = patch_size * 2
    ...
    def __getitem__(self, index):
        ...
        seq = normalize_raw_torch(pack_gbrg_raw_torch(seq))
        seqn = normalize_raw_torch(pack_gbrg_raw_torch(seqn))
        noise = generate_noisy_raw(seqn, a_list[iso_index], g_noise_var_list[iso_index])
        # std_p, std_g = generate_noisy_raw(seqn, a_list[iso_index], g_noise_var_list[iso_index])
        # print(torch.std(noise))
        # print(torch.std(seqn-seq))
        a = torch.tensor(a_list[iso_index], dtype=torch.float32).view((1, 1, 1, 1)) / (2 ** 12 - 1 - 240)
        b = torch.tensor(g_noise_var_list[iso_index], dtype=torch.float32).view((1, 1, 1, 1)) / ((2 ** 12 - 1 - 240) ** 2)
        # noise = generate_noisy_raw(gt_raw, a, b)
        return {'seq': seq,
                # 'seqn': seqn, 'iso': iso, 'a': a, 'b': b, 'std_p': std_p, 'std_g': std_g}
                'seqn': seqn, 'iso': iso, 'a': a, 'b': b,  'noise': noise}

    def __len__(self):
        return self.patches_per_epoch


class CRVDTestR2RDataset(Dataset):
    def __init__(self, CRVD_path):
        ...
        for iso in iso_list:
            for scene_id in self.scene_id_list:
                self.seqs['%d_%d_clean' % (iso, scene_id)] = open_CRVD_seq_raw(os.path.join(self.CRVD_path, 'indoor_raw_gt/indoor_raw_gt_scene%d/scene%d/ISO%d' % (scene_id, scene_id, iso)),
                                                                               'frame%d_clean_and_slightly_denoised.tiff')
                self.seqs['%d_%d_noisy' % (iso, scene_id)] = open_CRVD_seq_raw(os.path.join(self.CRVD_path, 'indoor_raw_noisy/indoor_raw_noisy_scene%d/scene%d/ISO%d' % (scene_id, scene_id, iso)),
                                                                               'frame%d_noisy0.tiff')

    def __getitem__(self, index):
        iso_index = index // len(self.scene_id_list)
        iso = iso_list[iso_index]
        ...
        return {'seq': seq,
                'seqn': seqn,
                'iso': iso, 'a': a, 'b': b, 'scene_id': scene_id, 'noise': noise}
...
class CRVDOurdoorDataset(Dataset):
    def __init__(self, CRVD_path):
        ...
        self.iso = 25600
        for scene_id in self.scene_id_list:
            self.seqs['%d_%d_noisy' % (self.iso, scene_id)] = open_CRVD_seq_raw_outdoor(os.path.join(self.CRVD_path, 'outdoor_raw_noisy/outdoor_raw_noisy_scene%d/scene%d/iso%d' % (scene_id, scene_id, self.iso)),
                                                                                        'frame%d.tiff')
